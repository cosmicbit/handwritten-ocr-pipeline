...
# from handwriting_ocr_pipeline.language.correct_text import correct
...
input = INPUTS_DIR / settings.inputFile
...
def pdf_to_numpy_list(pdf_path, zoom=2.0):
    """
    Converts PDF pages into a list of NumPy arrays (RGB).
    Matches the format: np.array with shape (H, W, 3).
    """
    image_list = []
    
    try:
        doc = fitz.open(pdf_path)
        # Matrix for scaling (resolution)
        mat = fitz.Matrix(zoom, zoom)
        
        for page in doc:
            pix = page.get_pixmap(matrix=mat)
            
            # Direct conversion: Pixmap -> Buffer -> NumPy Array
            # pix.samples contains the raw RGB bytes
            img = np.frombuffer(pix.samples, dtype=np.uint8).reshape(pix.h, pix.w, 3)
            
            # Ensure it's a copy so it persists after 'pix' is cleared
...
def main():
    ...
    images = pdf_to_numpy_list(input)
    recognized_lines = []
